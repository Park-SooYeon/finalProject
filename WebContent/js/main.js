...
main.like = function(ele) {
	...
	nowEle = heart;
	if(heart.classList.contains("fa-heart-o")) { // 여행 폴더 선택할 모달창 열기
		heart.setAttribute("data-toggle", "modal");
		heart.setAttribute("data-target", "#exampleModalScrollable");
	} else if(heart.classList.contains("fa-heart")) { // 좋아요 해제
...
//모달창에서 추가 버튼 클릭시 작동
main.makeHeart = function() { // 추가버튼 클릭시 좋아요가 선택됨
...
main.insertFolder = function(ele) {
	let newele = "<div class='folder p-2' onclick='main.selectFolder(this)'><i class='fa fa-folder pr-2'></i>여행명4</div>";
